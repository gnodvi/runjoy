...
copy (Node *n)
{
  ...
  switch (n->op)
  { 
  case INTEGER_: temp->u.num = n->u.num;  break;
  case SET_:     temp->u.set = n->u.set;  break;
  case STRING_:  temp->u.str = n->u.str;  break;
  case FLOAT_:   temp->u.dbl = n->u.dbl;  break;
  case FILE_:    temp->u.fil = n->u.fil;  break;
  case LIST_:    temp->u.num = (long)copy(n->u.lis); break;
  default:
    temp->u.num = n->u.num; 
  }
  /* end of replacement */
...
gc2 (char *mess)
{
  ...
  if (tracegc > 0)
    printf ("gc - %d nodes inspected, %d nodes copied, clock: \n",
            nodesinspected,nodescopied/* ,this_gc_clock */);
  if (tracegc > 1)
...
/*----------------------------------------------------------------------------*/
Node *
newnode (Operator o, Types u, Node *r)
{
  Node *p;
...
readmodule_field (void)
{
  ...
  getsym ();
  if (sym != PERIOD) error ("period '.' expected after module name");
  else               getsym ();
...
/*----------------------------------------------------------------------------*/
void 
readfactor (void)	/* read a JOY factor  */
{
  D (printf ("rf - BEG \n"));
...
readterm (void)
{
  ...
  D (printf ("readterm - END \n"));
  D (printf ("\n"));
  return;
}
/*----------------------------------------------------------------------------*/
void 
writefactor (Node *n, FILE *stm)
{
  if (n == NULL)
