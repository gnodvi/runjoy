...
{   
  ...
  if (p && p->op == INTEGER_)   { t->tm_mon   = p->u.num - 1; POP(p); }
  if (p && p->op == INTEGER_)   { t->tm_mday  = p->u.num;     POP(p); }
  if (p && p->op == INTEGER_)   { t->tm_hour  = p->u.num;     POP(p); }
  if (p && p->op == INTEGER_)   { t->tm_min   = p->u.num;     POP(p); }
  if (p && p->op == INTEGER_)   { t->tm_sec   = p->u.num;     POP(p); }
  if (p && p->op == BOOLEAN_)   { t->tm_isdst = p->u.num;     POP(p); }
  if (p && p->op == INTEGER_)   { t->tm_yday  = p->u.num;     POP(p); }
  if (p && p->op == INTEGER_)   { t->tm_wday  = p->u.num;     POP(p); }
...
/*----------------------------------------------------------------------------*/
void fputchars_() /* suggested by Heiko Kuhrt, as "fputstring_" */
{   
  FILE *stm;
  TWOPARAMS ("fputchars");
...
long equal_aux(); /* forward */

/*----------------------------------------------------------------------------*/
int equal_list_aux (n1, n2)
Node *n1, *n2;
{
...
void case_()
{
  ...
  /*
    ...
    */
  if (n->next != NULL) { stk = stk->next->next; exeterm(n->u.lis->next); }
  else                 { stk = stk->next;       exeterm(n->u.lis);       }
 ...
 start:
  ...
  while (conts->u.lis != NULL) {
    ...
    default:
      ...
      //D (writefactor (dump1, stdout)); // ??  - вылетам в ошибку и повтор..
      //D (writefactor (stk, stdout));     // а так нормально..
  ...
  { 
    ...
    { 
      if (SAVED2->u.set & (1 << j)) { 
        stk = INTEGER_NEWNODE (j, SAVED3);
        exeterm (SAVED1->u.lis);
        if (stk->u.num)  yes_set = yes_set | (1 << j);
...
void tailrecaux()
{
 ...
 tailrec:
  ...
  if (result) exeterm (SAVED2->u.lis); 
  else
  { 
    exeterm (SAVED1->u.lis);
    goto tailrec; 
...
void condnestrecaux()
{
  ...
  stk = DMP2;
  dump3 = LIST_NEWNODE ((result ? DMP1->u.lis->next : DMP1->u.lis),
                        dump3);
  exeterm (DMP3->u.lis);
  DMP3 = DMP3->next;
  while (DMP3 != NULL)
...
static struct {
  char *name; void (*proc) (); char *messg1, *messg2; 
}
...
void inisymboltable (void)		/* initialise			*/
{
  ...
  { 
    ...
    D (printf ("entered %17s    in symbol table at    %ld = %ld\n", \
...
/*----------------------------------------------------------------------------*/
void 
make_manual (int style /* 0=plain, 1=HTML, 2=Latex */)
{
  int i; char * n;
